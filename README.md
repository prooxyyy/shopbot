...
2. Инлайновые кнопки были полностью изменены, теперь вместо текста - эмодзи.
...
4. Добавлена команда для просмотра всех пользователей `/users` (Синяя ссылка кликабельна)
...
3. Код стал более оптимизированным
...
`/getrazrab` - Выдать себе статус разработчика (читать установку)
...
# 🛠 Установка бота:
1. Скачать файлы, затем распаковать папку в любое удобное место.
2. Открыть папку, а затем файл `configure.py`
3. Найти бота  `@BotFather` в телеграмм, написать команду `/newbot`
4. Ввести имя и ник бота, получить токен
...
В строку `'token'` нужно вписать токен который мы получили ранее у `@BotFather`:
...
Переходим по ссылке в сообщении с токеном бота от `@BotFather`
...
Либо на VDS или на Выделенный сервер
