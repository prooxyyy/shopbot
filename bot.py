...
def editbuy_name_new_name(message):
	...
	if message.text == message.text:
		...
		for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {editbuynameidtovar}"):
			rmk = types.InlineKeyboardMarkup()
			item_yes = types.InlineKeyboardButton(text='Да', callback_data='editbuynewnametovaryes')
			item_no = types.InlineKeyboardButton(text='Отмена', callback_data='editbuynewnametovarno')
...
def editbuy_price_new_price(message):
	...
	if message.text == message.text:
		...
		for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {editbuypriceidtovar}"):
			rmk = types.InlineKeyboardMarkup()
			item_yes = types.InlineKeyboardButton(text='Да', callback_data='editbuynewpricetovaryes')
			item_no = types.InlineKeyboardButton(text='Отмена', callback_data='editbuynewpricetovarno')
			rmk.add(item_yes, item_no)
...
def editbuy_tovar_new_tovar(message):
	...
	if message.text == message.text:
		...
		for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {editbuytovaridtovar}"):
			rmk = types.InlineKeyboardMarkup()
			item_yes = types.InlineKeyboardButton(text='Да', callback_data='editbuynewtovartovaryes')
			item_no = types.InlineKeyboardButton(text='Отмена', callback_data='editbuynewtovartovarno')
			rmk.add(item_yes, item_no)
	...
	if message.text == message.text:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
			for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {removeidtovar}"):
				rmk = types.InlineKeyboardMarkup()
				item_yes = types.InlineKeyboardButton(text='Да',callback_data='removebuytovaryes')
				item_no = types.InlineKeyboardButton(text='Отмена',callback_data='removebuytovarno')
				rmk.add(item_yes, item_no)
...
def buy(message):
	...
	for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
		for infoshop in sql.execute(f"SELECT * FROM shop"):
			text += f"{infoshop[0]}. {infoshop[1]}\nЦена: {infoshop[2]}\n\n"
		rmk = types.InlineKeyboardMarkup()
		item_yes = types.InlineKeyboardButton(text='Да', callback_data='firstbuytovaryes')
		item_no = types.InlineKeyboardButton(text='Отмена', callback_data='firstbuytovarno')
		rmk.add(item_yes, item_no)
		msg = client.send_message(cid, f'{text}\n\n*Вы хотите перейти к покупке товара?*',parse_mode='Markdown',reply_markup=rmk)
...
def teh_next(message):
	...
	if message.text == message.text:
		...
		rmk = types.InlineKeyboardMarkup()
		item_yes = types.InlineKeyboardButton(text='Отправить',callback_data='tehsend')
		item_no = types.InlineKeyboardButton(text='Отмена',callback_data='tehno')
		rmk.add(item_yes, item_no)
...
def sendmsgtouser_next_text(message):
	...
	if message.text == message.text:
		...
		rmk = types.InlineKeyboardMarkup()
		item_yes = types.InlineKeyboardButton(text='Да',callback_data='sendmsgtouseryes')
		item_no = types.InlineKeyboardButton(text='Отмена',callback_data='sendmsgtouserno')
		rmk.add(item_yes, item_no)
